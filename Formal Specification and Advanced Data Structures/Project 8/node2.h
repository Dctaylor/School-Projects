...
//
//   const node* list_search(const node* head_ptr, const node::Item& target)
...
#pragma mark - Linked List Functions
    ...
    template <class Item>
    Item list_remove_at(coen70_lab6B::node<Item>*& head_ptr, size_t position);
    template <class Item>
    coen70_lab6B::node<Item>* list_copy_segment(node<Item>* head_ptr, size_t start, size_t finish);
}
...
#pragma mark - Node Iterator
    ...
    {
    public:
        node_iterator(coen70_lab6B::node<Item>* initial = NULL) {
            current = initial;
        }
        typename coen70_lab6B::node<Item>::Item& operator *( ) const {
    ...
    private:
        coen70_lab6B::node<Item>* current;
    };
...
#pragma mark - Node Iterator
    ...
    {
    public:
        const_node_iterator(const coen70_lab6B::node<Item>* initial = NULL) {
            current = initial;
        }
        const typename coen70_lab6B::node<Item>::Item& operator *( ) const {
            // Dereference, gets value of current
            return current->data();
        }
    ...
    private:
        const coen70_lab6B::node<Item>* current;
    };
#include "node2.cxx"
#endif
